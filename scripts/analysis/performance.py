...
from scripts.data.fetch_index import INDEX_MAP
...
def bull_bear_analysis(
    ...
    bull_fund = (
        aligned["fund"][bull_mask].mean() * 252 * 100 if bull_mask.sum() > 0 else None
    )
    bull_bench = (
        aligned["bench"][bull_mask].mean() * 252 * 100 if bull_mask.sum() > 0 else None
    )
    bear_fund = (
        aligned["fund"][bear_mask].mean() * 252 * 100 if bear_mask.sum() > 0 else None
    )
    bear_bench = (
        aligned["bench"][bear_mask].mean() * 252 * 100 if bear_mask.sum() > 0 else None
    )
...
def _print_report(report: dict, bm_name: str) -> None:
    """打印业绩归因报告（从 main() 提取）。"""
    print(
        f"  基金: {report['fund_code']}  |  基准: {bm_name} ({report['benchmark_code']})"
    )

    print("\n[CHART] CAPM 归因:")
    if report.get("beta") is not None:
        print(
            f"  Alpha(年化): {report['alpha_annual']}% "
    ...
    if bb.get("upside_capture") is not None:
        print("\n[CHART] 牛熊市表现:")
        print(
            f"  上涨市: 基金 {bb['bull_fund_return']}%  |  基准 {bb['bull_bench_return']}%"
        )
        print(
            f"  下跌市: 基金 {bb['bear_fund_return']}%  |  基准 {bb['bear_bench_return']}%"
        )
        print(
            f"  上涨捕获率: {bb['upside_capture']}%  "
            f"({'涨时跟得上' if bb['upside_capture'] > 90 else '涨时跑输'})"
        )
        print(
            f"  下跌捕获率: {bb['downside_capture']}%  "
    ...
    print("\n[TIP] 解读指南:")
    print("  1. Alpha>0 且显著 → 基金经理有选股/择时能力")
    print("  2. Beta<1 → 防御型，熊市可能跌得少")
    print("  3. IR>0.5 → 主动管理能力优秀")
    print("  4. 上涨捕获率>100且下跌捕获率<100 → 理想的主动基金")
    print("  5. R²过低时Alpha不可靠，改用IR评估")
...
def main():
    ...
    _print_report(report, report["benchmark_name"])
