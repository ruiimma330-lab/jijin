...
def get_stock_daily(
    code: str, days: int = 180, adjust: str = "qfq",
) -> pd.DataFrame:
    """获取个股日线数据。
    ...
    if not fin.empty:
        ...
        for col in fin.columns:
            ...
            if "营收" in str(col) and "增长" in str(col):
                revenue_growth = float(fin[col].iloc[0]) if pd.notna(fin[col].iloc[0]) else None
            if "净利润" in str(col) and "增长" in str(col):
                profit_growth = float(fin[col].iloc[0]) if pd.notna(fin[col].iloc[0]) else None
...
def get_sector_stocks(sector_name: str) -> pd.DataFrame:
    ...
    try:
        ...
        cols = [c for c in ["code", "name", "price", "change_pct", "pe"]
                if c in df.columns]
        return df[cols]
    except Exception as e:
