...
def render():
    st.title("📈 业绩归因分析")
    st.markdown(
        "拆解基金收益来源：哪些来自市场（Beta），哪些来自基金经理能力（Alpha）。"
    )

    col1, col2 = st.columns(2)
    with col1:
        fund_code = st.text_input(
            "基金代码", value="110020", max_chars=6, placeholder="如 110020"
        )
    with col2:
    ...
    if report.get("beta") is not None:
        ...
        with col1:
            ...
            st.metric(
                "Alpha (年化)", f"{alpha}%", "超额收益" if alpha > 0 else "跑输基准"
            )
        with col2:
            beta = report["beta"]
    ...
    with col1:
        ...
        st.metric(
            "信息比率", f"{ir}", "优秀" if ir > 0.5 else ("良好" if ir > 0 else "跑输")
        )
    with col2:
